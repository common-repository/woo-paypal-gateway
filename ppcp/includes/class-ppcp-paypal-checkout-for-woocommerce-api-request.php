...
class PPCP_Paypal_Checkout_For_Woocommerce_API_Request {
    ...
    public function __construct() {
        ...
        if ($this->is_sandbox) {
            ...
            $this->client_id = $this->settings->get('rest_client_id_sandbox');
            ...
            $this->client_id = $this->settings->get('rest_client_id_live');
            $this->secret_key = $this->settings->get('rest_secret_id_live');
            $this->basicAuth = base64_encode($this->client_id . ":" . $this->secret_key);
            $this->access_token = get_transient('ppcp_live_access_token');
    ...
    public function ppcp_load_class() {
        try {
            if (!class_exists('PPCP_Paypal_Checkout_For_Woocommerce_Response')) {
                include_once WPG_PLUGIN_DIR . '/ppcp/includes/class-ppcp-paypal-checkout-for-woocommerce-response.php';
            }
                ...
                try {
                    ...
                    if (!empty($api_response['access_token'])) {
                        if ($this->is_sandbox) {
                            ...
                            set_transient('ppcp_live_access_token', $api_response['access_token'], 29000);
                        }
